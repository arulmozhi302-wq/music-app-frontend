...
export default function SongCard({ song, showActions = true, onAddToPlaylist }) {
  ...
  return (
    <>
      <div className="group flex items-center gap-4 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
        <button
          type="button"
          onClick={() => play(song)}
          className="relative w-14 h-14 rounded-lg overflow-hidden shrink-0 flex items-center justify-center bg-surface-800"
        >
          <img src={song.coverUrl || 'https://picsum.photos/112/112'} alt="" className="w-full h-full object-cover" />
          <span className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <span className={`w-10 h-10 rounded-full flex items-center justify-center ${isPlaying ? 'bg-brand-500' : 'bg-white/90 text-black'}`}>
        ...
        <div className="text-sm text-gray-500 shrink-0">{formatDuration(song.duration || 0)}</div>
        {showActions && (
          <div className="flex items-center gap-1 shrink-0">
            ...
            {onAddToPlaylist && (
              <button type="button" onClick={() => onAddToPlaylist(song)} className="p-2 text-gray-500 hover:text-white rounded-lg hover:bg-white/10" title="Add to playlist">
