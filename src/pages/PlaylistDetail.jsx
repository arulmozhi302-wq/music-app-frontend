...
export default function PlaylistDetail() {
  ...
  const coverUrl = playlist.coverUrl || songs[0]?.coverUrl || 'https://picsum.photos/400/400';

  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex gap-6 items-end">
        <div className="w-48 h-48 rounded-xl overflow-hidden bg-surface-800 shrink-0">
      ...
      <div className="space-y-1">
        {songs.map((song) => (
          <div key={song._id} className="group flex items-center gap-4">
            <div className="flex-1 min-w-0">
              <SongCard song={song} showActions onAddToPlaylist={isOwner ? () => setAddSongModal(song) : undefined} />
            </div>
            {isOwner && (
              <button
                type="button"
